 ...
 * "File Transfer Protocol (FTP)"</a> for receiving FTP files. Both active and passive modes are supported.
 * 
...
public class FtpServerHandler extends SimpleChannelInboundHandler<String> {
	 ...
	 *            passive IP address
	 * @param lowestPassivePort
	 *            lowest bound of passive ports range
	 * @param highestPassivePort
	 *            highest bound of passive ports range
	 * @param passiveOpenAttempts
	 *            number of ports to choose for passive socket open before reporting error
	 */
	public FtpServerHandler(DataReceiver receiver, InetAddress passiveAddress, int lowestPassivePort,
			int highestPassivePort, int passiveOpenAttempts) {
	 ...
	 *            number of ports to choose for passive socket open before reporting error
	 */
	public FtpServerHandler(DataReceiver receiver, byte[] passiveAddress, int lowestPassivePort,
			int highestPassivePort, int passiveOpenAttempts) {
		if (null == receiver)
			throw new IllegalArgumentException("Provided receiver is null");
		this.receiver = receiver;
		if (null == passiveAddress)
			throw new IllegalArgumentException("Provided passiveAddress is null");
		this.passiveAddress = passiveAddress;
		if (lowestPassivePort <= 0 || lowestPassivePort >= 1 << 16)
			throw new IllegalArgumentException("Provided lowestPassivePort: [" + lowestPassivePort
					+ "] ia out of valid range");
		if (highestPassivePort <= 0 || highestPassivePort >= 1 << 16)
			throw new IllegalArgumentException("Provided highestPassivePort: [" + highestPassivePort
					+ "] ia out of valid range");
		if (lowestPassivePort > highestPassivePort)
			throw new IllegalArgumentException("Provided lowestPassivePort: [" + lowestPassivePort
					+ "] must be not greater than " + "highestPassivePort: [" + highestPassivePort + "]");
		this.lowestPassivePort = lowestPassivePort;
		this.highestPassivePort = highestPassivePort;
		if (passiveOpenAttempts <= 0)
			throw new IllegalArgumentException("Provided passiveOpenAttempts: [" + passiveOpenAttempts
					+ "] must be positive");
	...
	public void channelRead0(ChannelHandlerContext ctx, String payload) throws Exception {
		...
		if (message.length() < 3)
			send("501 Syntax error", ctx, message, "");
		...
		// dispatch
		if ("USER".equals(cmd))
			send("230 USER LOGGED IN", ctx, cmd, args);
		else if ("CWD".equals(cmd)) {
			curDir = args;
			send("250 CWD command successful", ctx, cmd, args);
		} else if ("PWD".equals(cmd))
			send("257 \"" + curDir + "\" is current directory", ctx, cmd, args);
		else if ("MKD".equals(cmd))
			send("521 \"" + args + "\" directory exists", ctx, cmd, args);
		else if ("DELE".equals(cmd))
			send("550 " + args + ": no such file or directory", ctx, cmd, args);
		else if ("RMD".equals(cmd))
			send("550 " + args + ": no such file or directory", ctx, cmd, args);
		else if ("RNFR".equals(cmd))
			send("350 File exists, ready for destination name", ctx, cmd, args);
		else if ("RNTO".equals(cmd))
			send("250 RNTO command successful", ctx, cmd, args);
		else if ("SYST".equals(cmd))
			send("215 UNIX Type: Java custom implementation", ctx, cmd, args);
		else if ("NOOP".equals(cmd))
			send("200 OK", ctx, cmd, args);
			...
			send("500 Command unrecognized", ctx, cmd, args);
		lastCommand = cmd;
	}
	...
	protected void type(ChannelHandlerContext ctx, String args) {
		if ("I".equals(args))
			send("200 Type set to IMAGE NONPRINT", ctx, "TYPE", args);
		else if ("A".equals(args))
			send("200 Type set to ASCII NONPRINT", ctx, "TYPE", args);
		else
			send("504 Command not implemented for that parameter", ctx, "TYPE", args);
	}
	...
	protected void port(ChannelHandlerContext ctx, String args) {
		...
		if (null == addr)
			send("501 Syntax error in parameters or arguments", ctx, "PORT", args);
		else if (null != this.activeSocket && !activeSocket.isClosed())
	...
	protected void pasv(ChannelHandlerContext ctx, String args) throws InterruptedException {
		for (int i = 0; i < passiveOpenAttempts; i++) {
			...
			try {
				...
				send(String.format("227 Entering Passive Mode (%d,%d,%d,%d,%d,%d)", passiveAddress[0],
						passiveAddress[1], passiveAddress[2], passiveAddress[3], part1, part2), ctx, "PASV", args);
				break;
			} catch (IOException e1) {
		...
		if (null == this.passiveSocket)
			send("551 Requested action aborted", ctx, "PASV", args);
	}
	...
	protected void stor(ChannelHandlerContext ctx, String args) {
		if ("PORT".equals(lastCommand)) {
			if (null == this.activeSocket)
				send("503 Bad sequence of commands", ctx, "STOR", args);
	...
	protected void quit(ChannelHandlerContext ctx, String args) {
		send("221 Service closing control connection.", ctx, "QUIT", "");
		closeActiveSocket();
		closePassiveSocket();
		ctx.close();
	}
